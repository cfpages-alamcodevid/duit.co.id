...
if __name__ == "__main__":
    ...
    # Search specifically for ebook-formatter
    found = False
    for m in missing:
        if m['slug'] == 'ebook-formatter':
            print(f"CONFIRMED MISSING: {m['id']} - {m['title']} ({m['slug']})")
            found = True
            break
    
    if not found:
        print("Article 'ebook-formatter' is NOT missing research (or not marked as 📋 in catalog).")
