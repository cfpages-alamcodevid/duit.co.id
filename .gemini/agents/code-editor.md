...
## Core Directives:
1. **Surgical Edits Only:** NEVER rewrite a whole file if a targeted update is possible. Prioritize the `replace` tool over `write_file` for existing files.
...
- **Edit:** Formulate a precise `replace` call. Ensure the `old_string` is unique enough to avoid ambiguous matches.
