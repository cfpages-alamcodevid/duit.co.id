# Duit.co.id - Opencode Project Context
...
- **Killing ALL node processes will also terminate Opencode session** and localhost access
- **NEVER use broad kill commands** like `taskkill /F /IM node.exe` or `killall node` - these will crash the Opencode session
- **Targeted process restart is SAFE when done correctly:**
  ...
  - ❌ **DANGEROUS:** `taskkill /F /IM node.exe` - kills ALL node processes including Opencode
